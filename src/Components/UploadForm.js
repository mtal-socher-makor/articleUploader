...
function UploadForm({ handleCloseForm, open, newArticle }) {
  ...
  const handleChange = (value, fieldName) => {
    ...
    // if(fieldName === "date"){
    //   const timeStemp = value.getTime()/1000
    //   console.log(timeStemp , "IS HERE");
    //   setUploadForm({ ...uploadForm, [fieldName]: timeStemp });
    // }else{
  ...
  const sendPublication = async () => {
    ...
    console.log('uploadFoformToSendrm', formToSend);
        ...
        <Grid item xs={12}>
          <Grid container direction="column" alignItems="center">
            <Grid item xs={10} className={classes.inputsGrids}>
              <TextInputUnit name="title" label="Title" value={uploadForm.title || ''} onChange={(e) => handleChange(e.target.value, 'title')} error={errors.title} />
            </Grid>
            <Grid item xs={10} className={classes.inputsGrids}>
            ...
            <Grid item xs={10} className={classes.inputsGrids}>
              <FileInput setUploadForm={setUploadForm} uploadForm={uploadForm} setValidationResult={setValidationResult} errors={errors} setErrors={setErrors} />
            </Grid>
          </Grid>
